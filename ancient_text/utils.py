...
def cleaning_pdf_output(pdf_string):
    # remove numbers
    pdf_string = ''.join([i.lower() for i in pdf_string if not i.isdigit()])
    # join lines
    pdf_string = ' '.join(pdf_string.splitlines())
    # clean out 
    for ch in ['.', ',', ':', ';', '*', '{', '}', '[', ']', '"', "appendix"]:
        if ch in pdf_string:
            pdf_string = pdf_string.replace(ch, '')
